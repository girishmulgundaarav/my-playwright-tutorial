...
async function selectJQueryDate(page: Page, targetYear: string, targetMonth: string, targetDate: string, isFuture: boolean) {
...
```javascript
async function selectDropdownDate(page: Page, targetYear: string, targetMonth: string, targetDate: string) {
  // 1. Direct option select for Year
  await page.locator('select.ui-datepicker-year').selectOption({ label: targetYear });

  // 2. Direct option select for Month
  await page.locator('select.ui-datepicker-month').selectOption({ label: targetMonth });
...
test('Booking.com Date Selection', async ({ page }) => {
  ...
  while (true) {
    const monthYearHeader = await page.locator("h3[aria-live='polite']").nth(0).innerText();
  ...
  const checkinDays = await page.locator('table.b8fcb0c66a tbody').nth(0).locator('td').all();
...
test('Dummy Ticket Booking Form', async ({ page }) => {
  ...
  await page.locator('select[data-handler="selectYear"]').selectOption("2001");
  await page.locator('select.ui-datepicker-month').selectOption("Mar");
  ...
  await page.locator('select[data-handler="selectYear"]').selectOption("2025");
  await page.locator('select[aria-label="Select month"]').selectOption("Nov");
  
  const departCells = await page.locator('table.ui-datepicker-calendar td a').all();
...
test('IRCTC Date Selection', async ({ page }) => {
  ...
  // Select 10th day
  const dateCells = await page.locator('table.ui-datepicker-calendar tbody td a').all();
  for (const cell of dateCells) {
